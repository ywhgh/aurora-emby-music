...
## 5. 对话脚本 C — 开场对话（推荐先用这个 · 唯一推荐版本）

> 这是和 codex 对话的"主线版本"，**其他几版（A / B / D）只做存档**。
> 任何新接手本项目的 AI 助手 / reviewer，只要把这一节整段贴给 codex 即可启动整个 M1-M7 计划。

```text
# 角色
你是 Emby Music Web 项目的高级代码审计与重构工程师。当前仓库工作目录就是项目根，分支是 docs/audit-2026-07（提交 9525600 + 0cf8659）。

# 第一步：自己读完整上下文
请你先在仓库里自己读这两份文档，**不要省略任何一节**：

1. docs/audit-2026-07.md
   2026-07-20 完成的代码审计索引。覆盖全部已识别的 P0 安全漏洞、P1/P2 中低危、清理 / 性能 / 状态层维护任务，以及 F1-F7 新功能；包含敏感字段清单、脱敏规则、M1-M7 里程碑顺序、验收约定、交付格式、禁区清单。

2. docs/codex-handoff-2026-07.md
   本次我们之间所有对话脚本 / 提示词的合订本（A 完整版、B 修正版、C 推荐开场对话、D 三轮拆解）。建议你重点看 §1 敏感字段规则和 §5 / §6 的开场对话范式。

两份文档你都要 cat 一下，确认行数（应是 270 + 514），如果不一致先告诉我。

# 第二步：用 8-15 行回我确认理解
读完请你**先不动代码**，回我一段简短的复述：
- (a) 全部任务 S1-S8 / R1-R6 / F1-F7 里你认为 P0 的有哪些？为什么排这个序？
- (b) M1 应该一次性发 1 个 PR 还是拆成多个？你的拆分逻辑是什么？
- (c) 哪些任务涉及"敏感字段"？你打算如何用 src/redact.js 兜底？
- (d) 你对 §0.1 / §0.2 敏感字段清单里那些字段，在仓库里现在会从哪里读出来、又写到了哪里？举一两个具体文件位置。

# 第三步（这一步才是开始动手）
我对你（a）-（d）的回答没有异议后，**第一刀只动 src/redact.js + scripts/smoke-check.js 的单元用例**。这是 M1 的共用底座，单独 PR。
完成这一刀后给我：
- 文件路径与 unified diff（每个文件 ≤ 60 行 diff）；
- npm run check 的预期输出片段；
- 这个 PR 命名建议（按 audit §0.3 的禁区清单检查你自己的 diff）。

# 你必须遵守的全局约束（再贴一次怕你忘）
1. 敏感字段（serverUrl / serverId / userId / accessToken / deviceId / deviceName / externalSourceApiUrl / lyricsSourceBridgeApiUrl / localStorage / IndexedDB 里任何 host）**严禁**：
   - 写进 src/config.js 的字面默认值；
   - 出现在 console.log / setMessage / 错误信息字符串里；
   - 出现在"复制诊断"输出里，除非用户在本会话主动要求并已确认；
   - 上报任何外部端点。
2. UI 显示 server URL 一律脱敏为 192.168.*.*:****；token 仅显示 ***<last4>。
3. 不新增 npm 依赖；不改 Emby 服务器契约；不在 README / CHANGELOG 重复罗列（统一写 RELEASE_NOTES.md 对应段）。
4. 每个 PR 单独可 git revert；禁止大爆炸重写。
5. 沙箱（vm / worker_threads）里禁止出现 process / globalThis / 原生 require。
...
2. 关键 diff（unified diff，最多 60 行）
3. 验证命令与预期输出片段
4. 是否破坏现有功能：是/否；若是，列出回退路径
5. 是否涉及敏感字段：是/否；若是，列出已经在哪走 src/redact.js

# 千万不要做
- 不要假设用户配置是什么值（不要写死公网 IP、不要假设备名）
- 不要把 accessToken / serverUrl 直接拼到日志或错误信息里
- 不要"顺手"写 README / CHANGELOG（写到 RELEASE_NOTES.md 的"未发布"段）
- 不要"顺手"加 ESLint / vite / 其他工具链
- 不要把 audit 文档里说"这里有 bug"就立刻改，先确认它是真的 bug，再动代码
- 任何"我猜是这样"的不确定点，先停下来问我

# 我对你的容忍度
- 任务理解错：可以；但**别不动代码前**就理解错；
- 实现细节：可以试错；但错就老实说哪步走偏了；
- 任务范围扩大：0 容差 — 一旦超出当前 PR，立刻叫停。

# 项目完成确认（重要：这是终止条件）
当且仅当满足以下**全部**条件时，才回复"项目计划已完成"：
- audit §2.1 / §2.2 / §2.3 全部任务（S1-S8 / R1-R6 / F1-F7）的对应 PR 已经合并到 master；
- RELEASE_NOTES.md 的"未发布"段已经全部填满，并把段标题改成对应的发布版本号（例如 `## 0.94.0`）；
- §验收约定的所有命令最后一次跑全绿，并把命令输出片段贴出来；
- 当前分支无未提交 / 未推送的 commits；
- 包版本号（package.json / src/config.js / sw.js / manifest.webmanifest / index.html 内 `<link>` 的 query 参数）已全部同步到目标版本。

未完成时只汇报进度，例如：
  "已完成 S1-S8 + R1-R4；R5 app.js 拆分进行中，R5a 已合并；剩余 R5b / R5c / R6 / F1-F7。"

禁止说"接近完成""差不多"这种模糊话；禁止把未达成的子任务算到完成度里。

在所有任务收口之前，**不要主动结束会话 / 不要说"任务完成"**；每完成一个里程碑停下来等用户 review。
```
...
| 场景 | 用谁 | 怎么给 |
|---|---|---|
| 默认 / 你希望它读仓库文件、自己规划、自己验证 | **C**（§5） | 整段贴上；告诉它 start now |
| codex 不读文件、只能看上下文窗口 | **A 或 B** | 整段贴上 + 也贴 audit 链接（链接到 docs/audit-2026-07.md） |
| codex 上下文吃不动 | **D**（§6） | 三轮逐条发 |
| 只想 review 一个具体小改动 | 不需要整版 | 直接说："改 src/redact.js，按 audit §S8 第一条" |

> **统一建议**：默认走 C（C 已经覆盖 read-and-plan-and-act + 项目完成确认，比 A/B 紧凑一截）。A / B 是历史版本，只做存档。
...
| 你看到 | 说明 | 你怎么回 |
|---|---|---|
| codex 复述 §0.x 的规则和对 §3 里程碑的理解（脚本 C 的"第二步"回复） | 它读懂了 | "OK，开干"（落到脚本 C 第三步） |
...
| codex 给出进度汇报 "已完成 S1-S8 + R1-R4；R5 进行中..." | 它在按里程碑收尾 | 按 audit §3 验收该里程碑；如果与进度吻合，回 "OK，继续下一里程碑" |
| codex 回复"项目计划已完成" | **触发终止条件校验** | **不要立即合并 / 不要立即结束会话**。按 §5 的"项目完成确认"五项条件逐条核验；任何一项未达成，反问 "§X 还没合并/没全绿，按里程碑 Mx 继续"。只有五项全过才能算收口 |
| codex 提了一个新需求（如想顺手改 index.html） | 偏离当前 PR 范围 | "这个放下一刀，本次只动 §S8 涉及的" |
