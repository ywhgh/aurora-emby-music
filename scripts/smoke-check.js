...
function checkLyrics() {
  ...
  assert(/function renderNowLyricFocus\(\) \{[\s\S]*?if \(!state\.currentTrack\) \{[\s\S]*?renderImmersiveMobileCurrentLyric\(null\);[\s\S]*?renderImmersiveDesktopCurrentLyric\(null\);[\s\S]*?if \(!state\.lyricLines\.length\) \{[\s\S]*?renderImmersiveMobileCurrentLyric\(null\);[\s\S]*?renderImmersiveDesktopCurrentLyric\(null\);/.test(app), "Empty lyric states should clear both mobile and desktop immersive current lyrics");
  assert(app.includes("function runBrowserSmokeLyricTrackSwitchScenario") && app.includes("trackSwitchResidue"), "Browser smoke hooks should exercise lyric ownership across a track switch");
  assert(browserSmoke.includes("desktop immersive lyric retained the previous track text"), "Browser smoke should reject previous-track lyric residue");
