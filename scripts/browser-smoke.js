...
function checkPageState(check, page) {
  ...
  if (!isMobileImmersiveLayoutCheck) {
    ...
    assert(desktopImmersiveLayout.before?.playButtonLoadingAnimationName === "playbackLoadingOrbit", `${label} desktop immersive play loading should use the original gradient orbit: ${JSON.stringify(desktopImmersiveLayout.before)}`);
    assert(desktopImmersiveLayout.before?.playButtonLoadingAnimationDuration !== "0s", `${label} desktop immersive play loading animation should keep a non-zero duration: ${JSON.stringify(desktopImmersiveLayout.before)}`);
    assert(/radial-gradient|conic-gradient/.test(desktopImmersiveLayout.before?.playButtonLoadingBackgroundImage || ""), `${label} desktop immersive play loading should render the gradient orbit: ${JSON.stringify(desktopImmersiveLayout.before)}`);
    assert(desktopImmersiveLayout.before?.playButtonLoadingBorderRadius === "50%", `${label} desktop immersive play loading should keep a circular arc: ${JSON.stringify(desktopImmersiveLayout.before)}`);
    assert(desktopImmersiveLayout.before?.playButtonLoadingMaskImage !== "none", `${label} desktop immersive play loading should keep the ring mask: ${JSON.stringify(desktopImmersiveLayout.before)}`);
    assert(desktopImmersiveLayout.before?.playButtonLoadingTransform !== "none", `${label} desktop immersive play loading orbit should rotate: ${JSON.stringify(desktopImmersiveLayout.before)}`);
  ...
  assert(moreActionSheet.lyricSettingsAutoTranslateChecked === false, `${label} lyric missing-translation setting should default off: ${JSON.stringify(moreActionSheet)}`);
  ...
  if (isMobileImmersiveLayoutCheck) {
    ...
    assert(mobileImmersiveLayout.before?.currentLyricBilingual === true, `${label} mobile immersive cover current lyric should render bilingual lyrics when translation exists: ${JSON.stringify(mobileImmersiveLayout.before)}`);
    assert(mobileImmersiveLayout.before?.currentLyricLineCount === 2, `${label} mobile immersive cover current lyric should keep original and translation as two lines: ${JSON.stringify(mobileImmersiveLayout.before)}`);
    assert(/Long cover lyric/.test(mobileImmersiveLayout.before?.currentLyricOriginalText || "") && /封面歌词翻译/.test(mobileImmersiveLayout.before?.currentLyricTranslatedText || ""), `${label} mobile immersive cover current lyric should expose original and translated text: ${JSON.stringify(mobileImmersiveLayout.before)}`);
    ...
    assert(mobileImmersiveLayout.before?.playButtonLoadingAnimationName === "playbackLoadingOrbit", `${label} mobile immersive play loading should use the original gradient orbit: ${JSON.stringify(mobileImmersiveLayout.before)}`);
    ...
    assert(/radial-gradient|conic-gradient/.test(mobileImmersiveLayout.before?.playButtonLoadingBackgroundImage || ""), `${label} mobile immersive play loading should render the gradient orbit: ${JSON.stringify(mobileImmersiveLayout.before)}`);
    assert(mobileImmersiveLayout.before?.playButtonLoadingBorderRadius === "50%", `${label} mobile immersive play loading should keep a circular arc: ${JSON.stringify(mobileImmersiveLayout.before)}`);
    assert(mobileImmersiveLayout.before?.playButtonLoadingMaskImage !== "none", `${label} mobile immersive play loading should keep the ring mask: ${JSON.stringify(mobileImmersiveLayout.before)}`);
    assert(mobileImmersiveLayout.before?.playButtonLoadingTransform !== "none", `${label} mobile immersive play loading orbit should rotate: ${JSON.stringify(mobileImmersiveLayout.before)}`);
    ...
    assert(mobileImmersiveLayout.afterTitleReturn?.view === "cover", `${label} tapping mobile lyric title/artist should return to cover view: ${JSON.stringify(mobileImmersiveLayout.afterTitleReturn)}`);
    assert(mobileImmersiveLayout.afterTitleReturn?.coverVisible === true && mobileImmersiveLayout.afterTitleReturn?.lyricVisible === false, `${label} mobile lyric title return should restore cover/visualizer stage: ${JSON.stringify(mobileImmersiveLayout.afterTitleReturn)}`);
    assert(mobileImmersiveLayout.afterTitleReturn?.titleRole === "button" && mobileImmersiveLayout.afterTitleReturn?.titleTabIndex === "0", `${label} mobile lyric title/artist return target should be keyboard reachable: ${JSON.stringify(mobileImmersiveLayout.afterTitleReturn)}`);
