...
function checkPageState(check, page) {
  ...
  if (isMobileImmersiveLayoutCheck) {
    ...
    assert(mobileImmersiveLayout.before?.playButtonLoadingAnimationName === "playbackLoadingWave", `${label} mobile immersive play loading should use the home-style equalizer wave: ${JSON.stringify(mobileImmersiveLayout.before)}`);
    ...
    assert(((mobileImmersiveLayout.before?.playButtonLoadingBackgroundImage || "").match(/linear-gradient/gi) || []).length === 4, `${label} mobile immersive play loading should render four equalizer bars: ${JSON.stringify(mobileImmersiveLayout.before)}`);
    assert(mobileImmersiveLayout.before?.playButtonLoadingBorderRadius === "0px", `${label} mobile immersive play loading should not retain the ring shape: ${JSON.stringify(mobileImmersiveLayout.before)}`);
    assert(mobileImmersiveLayout.before?.playButtonLoadingMaskImage === "none", `${label} mobile immersive play loading should not retain the ring mask: ${JSON.stringify(mobileImmersiveLayout.before)}`);
    assert(mobileImmersiveLayout.before?.playButtonLoadingTransform === "none", `${label} mobile immersive play loading should not rotate: ${JSON.stringify(mobileImmersiveLayout.before)}`);
